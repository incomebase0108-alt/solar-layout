...
import { deriveCostInputs, buildCostInput, computeCandidateCost, computeActiveCost, missingPcsPrices } from "./candidateCost";
...
describe("missingPcsPrices（単価未登録の可視化）", () => {
  it("新設の台で単価未登録の機種名を返す（既設の台・単価ありは除外）", () => {
    const units = [
      { id: "u1", pcsId: "huawei", count: 2, kind: "new" }, // 単価あり
      { id: "u2", pcsId: "omron", count: 1, kind: "new" }, // 単価なし → 対象
      { id: "u3", pcsId: "omron", count: 1, kind: "existing" }, // 既設 → 除外
    ] as never;
    expect(missingPcsPrices(units, pcsList)).toEqual(["OMRON KPV-A55"]);
  });

  it("kind 未指定は新設扱い、マスタに無い機種は専用ラベル、重複はまとめる", () => {
    const units = [
      { id: "u1", pcsId: "omron", count: 1 },
      { id: "u2", pcsId: "omron", count: 2 },
      { id: "u3", pcsId: "ghost", count: 1 },
    ] as never;
    expect(missingPcsPrices(units, pcsList)).toEqual(["OMRON KPV-A55", "マスタ未登録の機種"]);
  });

  it("全台単価ありなら空", () => {
    const units = [{ id: "u1", pcsId: "huawei", count: 3, kind: "new" }] as never;
    expect(missingPcsPrices(units, pcsList)).toEqual([]);
    expect(missingPcsPrices(undefined, pcsList)).toEqual([]);
  });
});
